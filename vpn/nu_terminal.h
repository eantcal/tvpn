...
public:
   ...
   void move_cursor_next() noexcept
   {
      ++ _cursor;
   ...
   void move_cursor_prev() noexcept
   {
      if ( empty() ) return;
   ...
   void add_item( const std::string & line ) noexcept
   {
      push_back( line );
      assert( ! empty() );
      _cursor = size();
   ...
   svec_t get_history() const noexcept
   {
      return *this;
   }
...
public:
    ...
    */
   virtual int get_line( std::string& line, bool return_on_len_max = false ) noexcept;
